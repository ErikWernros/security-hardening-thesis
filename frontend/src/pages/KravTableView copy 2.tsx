...
export const KravTableView = ({ styckeId }: { styckeId: number }) => {
  ...
  // --vh fix for mobile browsers (proper viewport height)
  useEffect(() => {
    // Update CSS var on resize/orientation change
    const setVh = () => {
      // Use innerHeight instead of 100vh to avoid URL bar issues on mobile
  ...
  return (
    <section
      // Mobile-first container: adaptive min-height (~78vh) + fluid padding
      style={{ minHeight: 'calc(var(--vh, 1vh) * 78)' }}
      className='
  ...
  return (
    <div
      className='
        ...
        rounded-xl md:rounded-none bg-card/40 md:bg-transparent
      '
    >
      {/* Kod */}
